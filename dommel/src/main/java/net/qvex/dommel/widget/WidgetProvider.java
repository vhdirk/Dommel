 ...
 *
 */
public class WidgetProvider extends AppWidgetProvider //implements DataServiceResultReceiver.Receiver
{
    // log tag
    ...
    public void onReceive(Context context, Intent intent)
    {
        ...
        else if (action.equals(DommelDataService.MESSAGE_STATUS))
        {
            int resultCode = intent.getIntExtra(DommelDataService.FIELD_STATUS, -1);
            boolean manualUpdate = intent.getBooleanExtra(DommelDataService.FIELD_MANUAL, false);
            ...
            switch (resultCode) {
                case DommelDataService.STATUS_FINISHED:
                    ...
                    if(manualUpdate){
                        Toast.makeText(context, context.getResources().getString(R.string.success_message),
                                Toast.LENGTH_SHORT).show();
                    }

                    break;
                case DommelDataService.STATUS_ERROR:
                    Exception e = (Exception) intent.getSerializableExtra(DommelDataService.FIELD_EXCEPTION);
                    ...
                    if (e != null){
                        if(manualUpdate){
                            ...
                            Toast.makeText(
                                    context,
                                    context.getString(R.string.exception_message, e == null ? "null"
    ...
    private void updateData(Context context) {
        final Intent intent = new Intent(context, DommelDataService.class);
        intent.putExtra(DommelDataService.FIELD_MANUAL, true);
