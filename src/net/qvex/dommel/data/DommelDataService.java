...
public class DommelDataService extends WakefulIntentService
{
	// Android Intent actions
	public static final String UPDATE = "net.qvex.dommel.data.Update";
	public static final String USAGE_UPDATED = "net.qvex.dommel.data.UsageUpdated";
	public static final String EXCEPTION = "net.qvex.dommel.data.Exception";
	public static final String STOP_SERVICE = "net.qvex.dommel.data.StopService";
	public static final String SCHEDULE_SERVICE = "net.qvex.dommel.data.ScheduleService";
	...
	private static DommelDataService instance;
	...
	public DommelDataService() {
		...
		this.days_left = -1;
		this.reset_date = new Date();
	}

	/**
     * 
     */
	public static DommelDataService getInstance() {
		if (instance == null) {
			instance = new DommelDataService();
		}
		return instance;
	}
	...
	private void scheduleNextUpdate() {
		...
		long delay = Long.parseLong(prefs.getString("update_frequency", "-1"));

		if (delay > 0) {
			alarm.set(AlarmManager.ELAPSED_REALTIME_WAKEUP,
					SystemClock.elapsedRealtime() + delay, wakefulWorkIntent);

			if (this.getDebug()) {

				Log.d(DommelDataService.class.getSimpleName(),
						"Update scheduled in " + delay + "ms");
	...
	public boolean getData(String username, String password)
		...
		String res = this.httpPost(URL_LOGIN, urlParameters);
		...
		try {
			reset_date = df.parse(reset_date_str);
		...
		edit.putLong("reset_date", reset_date.getTime());
	...
	{	
		...
		exceptionBroadcast.putExtra(DommelDataService.EXCEPTION, e);
	...
	public static int calculateDaysLeft(Date reset_date) {
		Date today = new Date();
	...
	private void stopDataService() {
		...
		if (this.getDebug()) {
			Log.d(DommelDataService.class.getSimpleName(), "update canceled");
		}
